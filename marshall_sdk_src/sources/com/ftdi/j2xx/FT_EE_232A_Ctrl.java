...
class FT_EE_232A_Ctrl extends FT_EE_Ctrl {
    ...
    short programEeprom(FT_EEPROM ft_eeprom) {
        ...
        try {
            ...
            int stringDescriptor = setStringDescriptor(ft_eeprom.Product, iArr, setStringDescriptor(ft_eeprom.Manufacturer, iArr, 10, 7, true) + ft_eeprom.Manufacturer.length() + 2, 8, true) + ft_eeprom.Product.length() + 2;
            if (ft_eeprom.SerNumEnable) {
                setStringDescriptor(ft_eeprom.SerialNumber, iArr, stringDescriptor, 9, true);
                ft_eeprom.SerialNumber.length();
            }
