...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistAppExitInfoEvent(String str, ApplicationExitInfo applicationExitInfo, LogFileManager logFileManager, UserMetadata userMetadata) {
        if (applicationExitInfo.getTimestamp() >= this.reportPersistence.getStartTimestampMillis(str) && applicationExitInfo.getReason() == 6) {
            CrashlyticsReport.Session.Event eventCaptureAnrEventData = this.dataCapture.captureAnrEventData(convertApplicationExitInfo(applicationExitInfo));
    ...
    public Task<Void> sendReports(Executor executor) {
        ...
        while (it.hasNext()) {
            arrayList.add(this.reportsSender.sendReport(it.next()).continueWith(executor, new Continuation() { // from class: com.google.firebase.crashlytics.internal.common.-$$Lambda$SessionReportingCoordinator$E0EcUvGAcoqjPOmC0PORx8-bHg0
    ...
    private static List<CrashlyticsReport.CustomAttribute> getSortedCustomAttributes(Map<String, String> map) {
        ...
        Collections.sort(arrayList, new Comparator() { // from class: com.google.firebase.crashlytics.internal.common.-$$Lambda$SessionReportingCoordinator$qtBi7xGfVzzpU5rwhKDFPR0ZJaQ
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            strConvertInputStreamToString = convertInputStreamToString(applicationExitInfo.getTraceInputStream());
