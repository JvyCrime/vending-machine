...
public final class CCTDestination implements EncodedDestination {
    ...
    public byte[] asByteArray() {
        ...
        objArr[3] = str;
        return String.format("%s%s%s%s", objArr).getBytes(Charset.forName(UsbSerialDebugger.ENCODING));
    }

    public static CCTDestination fromByteArray(byte[] bArr) {
        String str = new String(bArr, Charset.forName(UsbSerialDebugger.ENCODING));
    ...
    static byte[] encodeString(String str) {
        return str.getBytes(Charset.forName(UsbSerialDebugger.ENCODING));
    }

    static String decodeExtras(byte[] bArr) {
        return new String(bArr, Charset.forName(UsbSerialDebugger.ENCODING));
    }
}
